...
export const postChangePassword = async (req,res)=>
{
    ...
    body : {oldPassword, newPassword, confirmation}}= req;
...
export const postLogin = async (req,res) => {
    ...
    if(!user)
    {
        return res.status(400).render("login",{pageTitle, errorMessage : "Account dose not exist."});
    }
...
export const finishGithubLogin = async (req,res)=>
{
    ...
    if("access_token" in tokenRequest){
        ...
        const emailObj = emailData.find(email => email.primary === true
            && email.verified === true);
        if (!emailObj)
        {
            return res.redirect("/login");
...
export const see = async (req,res) => {
    ...
    {pageTitle: `${user.name}의 Profile`, user});
};